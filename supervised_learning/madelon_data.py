...
dnn = MLPClassifier(hidden_layer_sizes=24, random_state=1, max_iter=10000, tol=0.00001, n_iter_no_change=100, verbose=VERBOSE)
dnn.fit(X_train, y_train)
# dnn.predict_proba(X_test[:1])

print('\nPredict DNN:\n')
# print(dnn.predict(X_test))
# print('Accuracy Train: {}'.format(accuracy_score(y_train, dnn.predict(X_train))))
# print('Accuracy Test: {}'.format(accuracy_score(y_test, dnn.predict(X_test))))

print('Train data Score: ', dnn.score(X_train, y_train))
print('Test data Score', dnn.score(X_test, y_test))

# KNN
knn = KNeighborsClassifier(n_neighbors=25, leaf_size=5, n_jobs=-1)
print('\nPredict KNN:\n')
